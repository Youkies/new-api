...
- 令牌 CRUD。
- 创建/编辑令牌时支持分组选择。
- 管理员创建/编辑令牌时可开启“调试 Key”，用于后台记录请求、上游请求、返回和错误；普通用户不可开启。调试 Key 也可调用 `/v1/debug/connectivity` 做用户端到服务器的连通性探测；如果额外开启“连通性测试 Key”，用户可直接把该 Key 复制到原软件里发起普通模型请求，服务端会短路返回检测完成，不走上游、不扣费。流式请求会默认保持约 60 秒 SSE 长流并持续发送进度；管理端调试记录页可设置流式时长、进度间隔和非流等待时长。
...
- KPay 原生 API 开启后，充值页会使用 `direct_qr` 模式在站内展示支付宝/微信二维码，不再把用户跳转到 KPay 收银台；支付完成后通过 `/api/kpay/notify` 回调入账，前端也可用 `/api/user/kpay/check` 做本次订单查单兜底。
- `uiweb` 会短期保存待支付 KPay 订单，移动端从支付 App 回到 `/topup` 或重新聚焦页面时自动恢复查单，减少“已支付但未到账”的人工处理。
- KPay 平台订单号会保存到本地充值单，即使浏览器没有携带平台单号，服务端也能优先用库内记录做查单兜底。
- 充值页下方展示最近充值订单；KPay 待支付/待到账订单提供“检查到账”按钮，用户可自行触发查单补偿并刷新余额。订单列表加载后会对可见 KPay 待支付订单静默查单一次，过期订单会刷新为“已过期”；早期没有保存平台订单号的 KPay 待支付单超过 15 分钟也会显示为“已过期”。KPay 待支付订单会短期保存在浏览器本地，用户保存二维码后即使关闭浏览器进程，再次进入充值页也会恢复扫码弹窗并立即查单一次；查单明确失败、取消或过期时会显示失败/已过期，不再继续停留在待确认语义。
