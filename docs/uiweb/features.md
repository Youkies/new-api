...
- 令牌 CRUD。
- 创建/编辑令牌时支持分组选择。
- 管理员创建/编辑令牌时可开启“调试 Key”，用于后台记录请求、上游请求、返回和错误；普通用户不可开启。调试 Key 也可调用 `/v1/debug/connectivity` 做用户端到服务器的连通性探测；如果额外开启“连通性测试 Key”，用户可直接把该 Key 复制到原软件里发起普通模型请求，服务端会短路返回检测完成，不走上游、不扣费。
