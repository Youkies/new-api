...
## 2026-05-19 ~ 2026-05-21 · 520 充值活动从「写死」→「后台化」两轮迭代

### 起点（5/19）
520 当天临时上线第一版"充值活动"，时间紧 → 选择把活动+4 个 SKU **写死在 `setting/promotion.go`**。
4 个 SKU 用 `5.20 / 13.14 / 52.0 / 99.99` 数字梗 + 方案 C 心理定价 (.99 结尾) + ≤ 9 折。

为什么这个版本能上线得快：
- 复用现有 KPay 链路（不重新做支付）
- 复用 top_ups 表（只加 `promotion_sku_id varchar(32)` 一列）
- 写死配置避开 admin UI / 表 schema / CRUD endpoints

### 上线后用户反馈链
关键反馈 → 多轮调整：
1. 「UI 太丑」 → 抽 PayMethodIcon 共享组件 + 选商品+底部支付方式按钮的两步式 UX
2. 「货币符号要全局统一」 → 替换硬编码 ¥ 为 `getCurrencyConfig().symbol`
3. 「金额改成 5.20 和 52.0」 → 加 `PriceDisplay` / `DeliveredDisplay` 字面量字段，避免 fmtAmount 吃掉尾零
4. 「Hero 在移动端错位」 → `flex flex-col sm:flex-row` + 去 truncate + 署名 whitespace-nowrap
5. 「支付条被小助手挡住」 → `showAssistantWidget={false}` + PaymentBar bg 改主题色渐变
6. 「移动端支付宝直跳，微信不行」 → 抄 TopUp 的 `isMobilePaymentContext` + `direct_pay_url` 直跳分支
7. 「banner 太雅」 → 撤回白底细条版本，恢复粉色渐变 + 立即查看胶囊

### 转折（5/20）—— 用户主动提出"后台化"
用户说"现在有时间了，来把活动充值拆解一下，做成后台可以管理的，新开分支，先给计划"。
这是从"快速交付节庆活动"切换到"长期工程基础设施"的关键时点。

设计抉择：
- **配置上 DB 不上 admin 上 (反 admin via 文件配置)**：表驱动 → admin 可改 / 历史可追 / 多活动并存
- **soft delete vs hard delete**：活动用 deleted_at（GORM 软删），SKU 用 enabled=0（不软删，因为 sku_key 是订单稳定引用）
- **进程内缓存 vs Redis**：30s TTL sync.RWMutex 进程内缓存。多 master 节点最多 30s 不一致 → 业务可接受 → 不引入 Redis 依赖
- **slug + sku_key 建后不可改**：前端 UI 显式 disabled。因为这俩是路由 + top_ups 引用的稳定 key
- **seed 触发：Unscoped count==0**：包括已软删的也算。admin 删 520 后重启不会被 seed 复活
- **入账路径 FindSkuByKey 不看 enabled**：让历史订单永远能解析（哪怕 SKU 被禁用）
- **decimal.Decimal 存价格**：避免 float 漂移；JSON 给前端时 float64，统计页用 StringFixed(2)

### 实施
5 个 Phase 一天内完成：
1. 后端骨架（model + 缓存 + seed）
2. Admin CRUD API（/api/ui/admin/promotions）
3. Classic /legacy/admin UI（单文件 ~900 行 JSX 避免拆得过散）
4. Banner 字段过滤（show_topup_banner）
5. 迁移文档 + 升级路径

### 沉淀文档
- `docs/uiweb/database-and-migrations.md` 加 promotion_campaigns / promotion_skus schema + 从硬编码版本升级的 4 步路径
- `kpay-adapter-pack.zip` 是这一轮副产物：用户希望把 KPay 接入经验打包共享给其他 new-api 用户

### 经验教训
- **节庆活动可以先写死再后台化**：用户压力大时不要追求一步到位
- **后台化不能动既有数据的引用键**：sku_key 必须保持稳定字符串
- **缓存层位置**：进程内 sync.Map 60s（高频入账路径） + 30s 全局活动缓存（banner / 落地页）。读写两类分开缓存
- **classic admin UI 不必拆得跟 redemption 那种七零八落**：单文件 600~900 行可读性反而更好
