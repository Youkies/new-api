...
export default defineConfig(({ envMode }) => {
  ...
  const serverUrl =
    ...
    'http://localhost:3000'
  const defaultUiBasePath =
    process.env.VITE_DEFAULT_UI_BASE_PATH ||
    env.rawPublicVars.VITE_DEFAULT_UI_BASE_PATH ||
    '/default'
  const defaultUiAssetPrefix = defaultUiBasePath.endsWith('/')
    ? defaultUiBasePath
    : `${defaultUiBasePath}/`
  ...
  return {
    ...
    output: {
      assetPrefix: defaultUiAssetPrefix,
