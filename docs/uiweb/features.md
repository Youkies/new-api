...
- 充值页下方展示最近充值订单；KPay 待支付/待到账订单提供“检查到账”按钮，用户可自行触发查单补偿并刷新余额。KPay 待支付订单会短期保存在浏览器本地，用户保存二维码后即使关闭浏览器进程，再次进入充值页也会恢复扫码弹窗并立即查单一次；查单明确失败、取消或过期时会显示失败/已过期，不再继续停留在待确认语义。
