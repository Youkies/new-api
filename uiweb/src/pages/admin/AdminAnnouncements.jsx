...
function formFromAnnouncement(item) {
  ...
  return {
    ...
    notify_level: item.notify_level || 'info',
...
function formToPayload(form) {
  return {
    ...
    notify_level: form.notify_level || 'info',
...
export default function AdminAnnouncements() {
  ...
  return (
    <ClayAdminShell
      title="公告管理"
      subtitle="管理公告历史、通知送达和强制确认。完整编辑会递增版本号，用户需重新确认。"
      actions={actions}
    >
      <div className="grid sm:grid-cols-3 gap-4 mb-5">
        <Stat label="公告总数" value={total || items.length} tone="blue" />
        <Stat label="强制确认" value={stats.force} tone="pink" />
        ...
        <div className="grid gap-4">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <Field label="通知级别">
              <ClaySelect
                ...
                onChange={(v) => updateForm('notify_level', v)}
              />
            </Field>
            <Field label="优先级">
              <ClayInput
          ...
          <div className="grid sm:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <ToggleField
              ...
              onChange={(v) => updateForm('notify_enabled', v)}
            />
            <ToggleField
              label="需确认"
              checked={form.require_ack}
              onChange={(v) => updateForm('require_ack', v)}
            />
