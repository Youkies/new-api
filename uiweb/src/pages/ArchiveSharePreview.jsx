  ...
  return (
    <ClayConsoleShell title="导入分享存档" subtitle="预览后再决定是否导入到自己的账户" compactHeader>
      ...
      {!loading && error && (
        <ClayCard className="!py-12 text-center">
          <div className="flex flex-col items-center gap-3">
            <AlertTriangle className="w-10 h-10 text-clay-yellow-300" />
      ...
      {!loading && !error && preview && (
        <>
          <ClayCard className="!p-5 mb-4">
            ...
            {preview.is_own && (
              <div className="mt-3 px-3 py-2 rounded-clay-sm bg-clay-yellow-50 text-clay-yellow-ink text-xs flex items-center gap-2">
          ...
          <ClayCard className="!p-0 mb-4 overflow-hidden">
            <div className="px-4 py-3 border-b border-clay-line/10 text-xs font-bold text-clay-faint">
              别名清单
            </div>
            <div className="divide-y divide-black/5">
              {(preview.aliases || []).map((a, idx) => (
                <div key={idx} className="px-4 py-3 flex items-center justify-between gap-3">
                  ...
                  {a.accessible ? (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-clay-pill text-[11px] font-bold bg-clay-green-100 text-clay-green-ink shrink-0">
                      <ShieldCheck className="w-3 h-3" /> 可用
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-clay-pill text-[11px] font-bold bg-clay-yellow-100 text-clay-yellow-ink shrink-0">
