...
- 每日积分封顶，默认 `3000`。
- 每周积分封顶，默认 `10000`。
- 开榜奖励倍率，使用百分比保存，例如 `300` 表示 3 倍。

审核与奖励：

- 用户必须有对应模型成功消费日志才能评价。
- 同一用户同一模型只保留一条评价，修改时只补发质量奖励差额。
- 管理员可以隐藏/公开评价，也可以精选评价。
- 精选评价会按后台配置发放精选奖励；取消精选不自动扣回历史奖励。
- 积分兑换额度时写入积分流水，并增加 `users.quota`。

接口：

- 用户侧：`GET /api/ui/model-reviews/rankings`
- 用户侧：`GET /api/ui/model-reviews`
- 用户侧：`GET /api/ui/model-reviews/eligibility`
- 用户侧：`POST /api/ui/model-reviews`
- 用户侧：`POST /api/ui/model-reviews/:id/helpful`
- 用户侧：`GET /api/ui/model-reviews/points`
- 用户侧：`POST /api/ui/model-reviews/points/redeem`
- 管理侧：`GET/PUT /api/ui/admin/model-reviews/settings`
- 管理侧：`GET /api/ui/admin/model-reviews`
- 管理侧：`PATCH /api/ui/admin/model-reviews/:id`
