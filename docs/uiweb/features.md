...
页面：`uiweb/src/pages/ModelReviews.jsx`

能力：

- 公开展示模型口碑榜，入口为 `/must-eat`。
- 榜单综合分结合五星评价、评价数量、评价质量、有帮助数、精选数和近 30 天使用热度。
- 登录用户只要对某个模型有一次成功消费日志，就可以评价该模型。
- 评价包含五星、使用场景、标签、优点、不足、一句话评价。
- 评价下方展示用户对该模型的已验证使用次数；用户可勾选隐藏真实次数，前台仅显示“已验证使用”。
- 用户可勾选匿名评价；匿名只影响前台展示，后台仍保留真实用户用于风控和奖励。
- 其他用户可以点“有帮助”，用于评价排序和追加积分奖励。
- 食评积分可兑换余额额度，兑换比例由后台配置。

奖励策略：

- 首次有效评价、质量奖励、有帮助奖励、管理员精选奖励均走食评积分账户和积分流水。
- 每日封顶、每周封顶、兑换比例、最低起兑、开榜倍率均可在后台修改。
- 同一用户同一模型只保留一条评价；修改评价只按质量分差额补发奖励，不重复发首次奖励。
- 管理员隐藏或精选评价不会公开用户日志内容，评价只展示模型名和用户主动填写的内容。
