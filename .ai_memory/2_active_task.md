...
- 日历格子改为 `aspect-square` 自适应正方形，空白日期也占位一致，移动端不再因固定高度/宽度导致错位。
- 签到金额从白色浮层改成格子内部轻量小字，使用更柔和的粉色系文字，不再叠加图标和白底胶囊，减少移动端拥挤感。
- 日历格子内金额单独格式化为纯数字（如 `0.53`），不显示 `+` 或货币符号；统计卡片与签到成功提示仍保留完整金额格式。

## 当前新增任务：关于页退款说明（2026-04-29）
### 本轮实现进度
- 关于页面新增固定“退款说明”区块，不受后台自定义 `about` HTML 是否存在影响。
- 规则：用户原因退款按“实付金额 - 已使用额度对应费用 - 手续费”计算，不包含赠送额度；商家原因服务异常经核实后全额退款；用户通过 QQ 联系管理员申请，3 个工作日内处理。

## 当前新增任务：AI 助手滚动遮挡与移动端下拉优化（2026-04-29）
### 本轮实现进度
- 目标：修复桌面端助手回复被底部 composer 遮挡、移动端模型选择列表看不全模型 ID、发送按钮文字挤成竖排、消息区能滚到半屏空白的问题。
- 布局：取消桌面端 composer 的绝对定位，改为 flex 底部正常流；消息滚动区去掉原本为绝对定位预留的大底部 padding，仅保留少量底部间距；短对话时消息列底部对齐，最后一条消息自然停在输入框上方。
- 选择器：自定义 clay 下拉弹层加宽，移动端模型弹层右对齐，选项允许 `break-all` 显示完整模型 ID，不再截断成看不清的半截。
- 按钮：发送按钮增加最小宽度、禁止换行并固定收缩行为，避免“发送/回复中”在移动端被挤成竖排。

## 当前新增任务：AI 助手选择框、余额提示与历史标题优化（2026-04-29）
### 本轮实现进度
- 目标：余额分组/模型选择框不能使用浏览器原生下拉样式；免费次数用完后的余额提示应只提醒一次；历史记录应在用户发送后立即有会话记录，并用用户问题生成简短标题。
- 前端：`AssistantClaySelect` 改为自定义 clay 弹层，选项面板使用黏土背景、圆角、阴影与滚动区域，不再显示系统原生 option 面板。
- 余额续聊：用户点击“使用余额继续”后进入 `balanceMode`，本次打开 AI 助手后续发送直接走余额续聊，不再重复展示免费次数耗尽提示；composer 显示“已确认使用余额续聊”的轻提示。
- 历史对话：发送前先调用 `createAssistantConversation` 预创建会话，标题由用户问题本地生成短标题（不额外调用总结模型）；即使后续免费限制或上游报错，历史列表也能出现会话入口。
- 诊断体验：历史列表读取/创建失败时不再静默显示“还没有历史对话”，而是显示具体错误，便于发现生产新表未创建等问题。
